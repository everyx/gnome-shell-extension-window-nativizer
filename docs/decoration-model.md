...
> **Mozilla Gecko / Firefox** (`libxul.so`) is **not** a provider. While Mozilla Bug 1850827 (enabled in Firefox 153+) injects an application-priority `GtkCssProvider` (`window.csd decoration { border-bottom-*-radius: %dpx; }`) and internal body clipping to achieve 4-corner decoration under default horizontal tab layouts, Firefox deliberately leaves its web content viewport rectangular to preserve web platform coordinate fidelity and controls. Under modern layouts such as Vertical Tabs, this results in an unclipped single square corner at the bottom-right. Treating `libxul.so` as a non-provider allows Window Nativizer to clip all four corners uniformly to the 15px Adwaita radius and apply our native ambient shadow (`shadowPad: 28`), establishing complete visual uniformity. (Note: On HDR displays, external clipping currently triggers Mutter's offscreen HDR color state limitation, tracked in Issue #21 and pending upstream resolution).
