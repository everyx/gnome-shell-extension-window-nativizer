...
> **libxul (Mozilla Gecko)**: While Firefox runs as a GTK3 client, Mozilla Bug 1850827 (enabled by default in Firefox 153+) circumvents GTK3's top-only decoration limitation: Gecko dynamically injects an application-priority `GtkCssProvider` (`window.csd decoration { border-bottom-*-radius: %dpx; }`), clips the window body via CSS `env(-moz-gtk-csd-titlebar-radius)`, and carves the four transparent corner rectangles out of its Wayland/X11 opaque region. Thus, Firefox natively provides full four-corner curvature and a matching CSD shadow. Treating `libxul.so` as a native-like provider avoids redundant external clipping shaders that otherwise trigger Mutter FP16 offscreen darkening on affected GPUs (issue #21).
