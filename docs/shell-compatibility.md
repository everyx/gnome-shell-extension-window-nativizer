...
| `win.is_maximized()` / `win.get_maximized()` | 45–51 (45–48 fallback) | 49–51 canonical `meta_window_is_maximized()`; 45–48 uses `win.get_maximized()` returning `MetaMaximizeFlags`. Polyfilled in `detector.isWindowMaximized()` |
...
| `win.begin_grab_op()` | 45–51 (dual signature) | 49–51 takes 4 args `(op, sprite, time, pos_hint)`; 45–48 takes 5 args `(op, device, sequence, time, pos_hint)`. Dispatched via try-fallback in `resizeBandActor.js` |
...
| `Clutter.Actor:set_cursor_type()` | 50–51 (45–49 degraded) | per-actor cursor introduced in Clutter 50; optional chained `child.set_cursor_type?.()` gracefully degrades on 45–49 without breaking resizing |
...
Audit of the Mutter and Shell C source code trees from 45.0 through 51.0 identifies four key architectural watersheds. The extension bridges each without platform branching:

| Feature / Subsystem | GNOME 45–48 | GNOME 49 | GNOME 50–51 | Extension Strategy |
|---|---|---|---|---|
| **Maximized Check** | `get_maximized()` (`MetaMaximizeFlags`) | `is_maximized()` (`gboolean`) | `is_maximized()` (`gboolean`) | `win?.is_maximized ? win.is_maximized() : win?.get_maximized?.()` boolean evaluation |
| **Grab Operation** | 5 args: `(op, dev, seq, time, pos)` | 4 args: `(op, sprite, time, pos)` | 4 args: `(op, sprite, time, pos)` | Try 4-arg sprite call; catch and fallback to 5-arg device/sequence call |
| **Actor Cursor** | Not available | Not available | `set_cursor_type(type)` | Optional chaining `child.set_cursor_type?.()`; cursor feedback degrades silently on 45–49 |
